...
# 开源项目文档vuepress-theme-hope

因为个人笔记的记录从原来的网盘保存逐渐演化为md文档本地线上同步，加之最近面试频繁用收集查看资料，而之前的蘑菇博客本地开发进展缓慢，效率低且占用资源，所以准备用一个项目将已有的所有md笔记集合为一个静态网站，更新且方便在线查看与维护。从mall的作者那里看到一个`vuepress-theme-hope`项目，简单地进行了构造，目前已经部署到http://note.strangest.cn上。

这里搭建过程参照https://mp.weixin.qq.com/s/SNViLxupNDNlM4wNctt8Qg 这里不记录如何搭建，而是本人做了什么额外的开发。